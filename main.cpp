...
extern void runChatServer(); // Forward declaration of the server function  
#include "LoginScreen.h"
#include "ChatRoomScreen.h"
 
int main(int argc, char *argv[])
{
    std::thread serverThread(runChatServer); // Start the chat server in a separate thread
 
    // FIX (Race condition): Give the server thread time to reach
    // io_context.run() and bind its acceptor. Without this, a user clicking
    // Connect immediately gets ECONNREFUSED → "Connection Failed" because the
    // listening socket doesn't exist yet. 100 ms is more than enough.
